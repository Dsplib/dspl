...
[y, n, m] = dspl_readbin('../dat/histogram_rand_y.bin');
[h, n, m] = dspl_readbin('../dat/histogram_rand_h.bin');
[n, x] = hist(y, length(h));
n = n/(length(y)*(x(2)-x(1)));
dspl_verification('dspl_histogram_norm for rand',  x+1i*n, h.');


[y, n, m] = dspl_readbin('../dat/histogram_randn_y.bin');
[h, n, m] = dspl_readbin('../dat/histogram_randn_h.bin');
[n, x] = hist(y, length(h));
n = n/(length(y)*(x(2)-x(1)));
