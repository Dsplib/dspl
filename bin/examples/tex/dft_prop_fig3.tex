...
\foreach \z in { 1, 2, ..., 7}
{	
	\draw [->, thin, draw = white!70!black] (\z * 0.25 , 0cm) -- (2 + \z * 0.25 , -3.5);
	\draw [->, thin, draw = white!70!black] (2+ \z * 0.25 , 0cm) -- (\z * 0.25 , -3.5);
...
\draw[->,thick] (-0.5cm,	0cm)		-- (5cm, 0cm)	node[below] {$k$};
...
\draw[->,thick] (-0.5cm,	-3.5cm)		-- (5cm, -3.5cm)	node[below] {$k$};
...
\foreach \z in { 1, 2, ..., 7}
{	
	\draw [thick, draw = red] (\z * 0.25 , 0cm) -- (\z * 0.25 , 2.0 - \z*0.25);
	\fill[fill = red] (\z * 0.25 , 2.0 - \z*0.25 ) circle(0.05cm);	
...
\end{tikzpicture}
\end {document}
%		\caption{Инверсия спектра сигнала}\label{dftprop:fig2}
%	\end{center}
%\end{figure}
