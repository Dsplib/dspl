...
В результате выполнения программы, в директории `dat/dtmf` будут созданы 16 файлов временных осциллограмм DTMF сигналов:`sym_x_time.txt`, где `x` обозначает один DTMF символ. 
<BR>
...
Кроме того в той же директории `dat/dtmf` будут созданы 16 файлов хранящих амплитуды спектральных отсчетов соответствующих частотам DTMF: `sym_x_ freq.txt`, где `x` обозначает один DTMF символ.<BR>

Символы [*] и [\#] обозначаются буквами `s` и `r`
соответственно.
<BR>

Временные осциллограммы DTMF сигналов, а также амплитуды спектральных отсчетов соответствующих частотам DTMF, полученные в результате работы программы могут быть построены на графиках.
